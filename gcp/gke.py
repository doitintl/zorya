...
class Gke(object):
    ...
    def change_status(self, to_status, tagkey, tagvalue):
        try:
            ...
            for cluster in clusters:
                if 'resourceLabels' in cluster and \
                    tagkey in cluster['resourceLabels'] and \
                    cluster['resourceLabels'][tagkey] == tagvalue:
                        ...
                        for instanceGroup in nodePool['instanceGroupUrls']:
                            ...
                            if int(to_status) == 1:
                                logging.info(
                                    ...
                                    "tagkey "
                                    "%s tagvalue %s",
                                    nodePool['name'], cluster['name'], tagkey,
                                    tagvalue)
                                res = GkeNodePoolModel.query(
                                    GkeNodePoolModel.Name ==
                                    node_pool_name).get()
                                logging.debug(res)
                                if not res:
                                    continue
    ...
    def list_clusters(self):
        ...
        result = self.gke.projects().locations().clusters().list \
